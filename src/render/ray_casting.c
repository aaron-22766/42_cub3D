...
static void	set_projection_values(t_game *game, t_render *render, t_ray *ray)
{
	...
	if (!render->ray_index) printf("After ray->pos.y: %lf\n", ray->pos.y); // Debug
	if (!render->ray_index) printf("After ray->pos.x: %lf\n", ray->pos.x); // Debug
...
void    render_raycast(t_game *game, t_render *render, t_ray *ray)
{
	...
	// }
	// if (ray->pw_top < 0) printf("ray->pw_top: %d\n", ray->pw_top); // Debug
	// if (ray->pw_bottom > game->image->height) printf("ray->pw_bottom: %d\n", ray->pw_bottom); // Debug
	// if (ray->pw_height > game->image->height) printf("ray->pw_height: %d\n", ray->pw_height); // Debug
	if (ray->pw_top < 0 || ray->pw_bottom > game->image->height || ray->pw_height < 0) {
		printf("ray->pw_height: %lf\n", ray->pw_height); // Debug
		printf("ray->pw_top: %lf\n", ray->pw_top); // Debug
		printf("ray->pw_bottom: %lf\n", ray->pw_bottom); // Debug
	}
	img_col = ray->pw_top;
	...
	while (img_col < (int)ray->pw_bottom)
	{
		...
		mlx_put_pixel(game->image, img_row, img_col, color);
		img_col++;
	}
	// if (render->ray_index == 0) print_pixel_rgba(color); // Debug
